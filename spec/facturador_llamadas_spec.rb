...
describe  do
  ...
  before(:each) do
    @costo_local_habil = CostoDeLlamada.new(0.2, ->(llamada) { llamada.es_dia_de_semana? & es_horario_laboral?})
    @costo_local_no_habil = CostoDeLlamada.new(0.1, ->(llamada){ llamada.es_fin_de_semana? || llamada.es_horario_no_laboral? })
